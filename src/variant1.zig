...
fn fbm6(vec: laf.Vec2) f32v {
    ...
    return t * laf.splat(1.1);
}

fn pattern(p: laf.Vec2) struct { f32v, laf.Vec2, laf.Vec2 } {
    // low frequency
    const q: laf.Vec2 = .{
        .x = laf.splat(0.5) + laf.splat(0.5) * fbm(4, .{ .x = p.x + laf.splat(1.1), .y = p.y + laf.splat(0.1) }),
        .y = laf.splat(0.5) + laf.splat(0.5) * fbm(4, .{ .x = p.x + laf.splat(5.1), .y = p.y + laf.splat(1.5) }),
    };

    // mid frequency
    const r: laf.Vec2 = .{
        .x = laf.splat(0.5) - laf.splat(0.5) * fbm6(.{ .x = p.x + laf.splat(4.1) * q.x, .y = p.y + laf.splat(4.1) * q.y }),
        .y = laf.splat(0.5) - laf.splat(0.5) * fbm6(.{ .x = p.x + laf.splat(4.1) * q.x, .y = p.y + laf.splat(4.1) * q.y }),
    };

    // high frequency
    const f = laf.splat(0.5) + laf.splat(0.5) * fbm(4, p.add(r.mul1(laf.splat(2.1))));
    return .{ f, r, q };
}
...
const ProcessingFunctor = struct {
    ...
    pub inline fn process(ctx: ProcessingFunctor, x: f32v, y: f32v) [3]u8v {
        ...
        const e = laf.splat(1.0) / ctx.scale;
        ...
        const normal = laf.Vec3.normalize(.{ .x = fex - f, .y = fey - f, .z = e });

        // we define a light direction
        const light = laf.Vec3.normalize(.{ .x = laf.splat(0.5), .y = laf.splat(-0.3), .z = laf.splat(-0.1) });
        // we compute the diffuse term
        const diff = std.math.clamp(laf.splat(0.5) + laf.splat(0.9) * laf.Vec3.dot(normal, light), laf.splat(0.0), laf.splat(1.0));
        const lin: laf.Vec3 = .{
            .x = (normal.z * laf.splat(0.2) + laf.splat(0.7)) + laf.splat(0.1) * diff,
            .y = (normal.z * laf.splat(0.2) + laf.splat(0.7)) + laf.splat(0.1) * diff,
            .z = (normal.z * laf.splat(0.2) + laf.splat(0.7)) + laf.splat(0.1) * diff,
        };
        col = col.mul(lin);
...
pub fn generate_image(allocator: std.mem.Allocator, width: u32, height: u32, time: f32) !std.ArrayList(u8) {
    ...
    const sin_time: f32v = @splat(@sin(time));

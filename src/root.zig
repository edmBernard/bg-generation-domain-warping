...
inline fn smoothstep(edge0: laz.InnerType, edge1: laz.InnerType, x: laz.InnerType) laz.InnerType {
    const t = std.math.clamp((x - edge0) / (edge1 - edge0), laz.toV(0.0), laz.toV(1.0));
    return t * t * (laz.toV(3.0) - laz.toV(2.0) * t);
}
...
inline fn supersmoothstep(edge0: laz.InnerType, edge1: laz.InnerType, x: laz.InnerType) laz.InnerType {
    const t = std.math.clamp((x - edge0) / (edge1 - edge0), laz.toV(0.0), laz.toV(1.0));
    return t * t * t * (laz.toV(10.0) - laz.toV(15.0) * t + laz.toV(6.0) * t * t);
}
    ...
    for (0..height) |i| {
        ...
        for (0..width / laz.vec_len) |j| {
            ...
            col = col.lerp(
                ...
                laz.toV(0.5) * smoothstep(laz.toV(1.1), laz.toV(1.3), @abs(r.x) + @abs(r.y)),
            );
            ...
            const temp = laz.Vec3.ones().add(col.mul1(laz.toV(-1.0)));

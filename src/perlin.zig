...
pub fn noise_gradient(p: laf.Vec2) f32v {
    ...
    // gradients
    const ga = hash.hash2(.{ .x = ix + laf.splat(0.0), .y = iy + laf.splat(0.0) });
    const gb = hash.hash2(.{ .x = ix + laf.splat(1.0), .y = iy + laf.splat(0.0) });
    const gc = hash.hash2(.{ .x = ix + laf.splat(0.0), .y = iy + laf.splat(1.0) });
    const gd = hash.hash2(.{ .x = ix + laf.splat(1.0), .y = iy + laf.splat(1.0) });
